...
describe('Room', () => {
  ...
  beforeEach(() => {
    room = new Room('Room A', 10000, 10); // 100.00€ con 10% descuento
    booking1 = new Booking('John Doe', 'john@example.com', new Date('2025-05-01'), new Date('2025-05-03'), 5, room);
    booking2 = new Booking('Jane Smith', 'jane@example.com', new Date('2025-05-04'), new Date('2025-05-05'), 0, room);
  ...
  test('occupancyPercentage returns correct value', () => {
    ...
    // 3 días ocupados (1-2, 4), 5 días en total
  ...
  test('static totalOccupancyPercentage works', () => {
    ...
    room2.bookings.push(new Booking('Mark', 'mark@mail.com', new Date('2025-05-02'), new Date('2025-05-04'), 0, room2));
    const start = new Date('2025-05-01');
    const end = new Date('2025-05-05');

    const total = Room.totalOccupancyPercentage([room, room2], start, end);
    expect(total).toBeGreaterThan(0); // Puedes ajustar el cálculo exacto si quieres precisión
  });
...
describe('Booking', () => {
  test('fee is calculated correctly with discounts', () => {
    const room = new Room('Room X', 10000, 10); // 100.00€
    const booking = new Booking('Client', 'client@mail.com', new Date('2025-05-01'), new Date('2025-05-04'), 5, room);
    // 3 noches → 100 x 3 = 300€, -10% room = 270€, -5% booking = 256.5€
